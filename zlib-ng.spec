...
CFLAGS="%{optflags} -fprofile-use=$(realpath %{name}-llvm.profdata)" \
CXXFLAGS="%{optflags} -fprofile-use=$(realpath %{name}-llvm.profdata)" \
LDFLAGS="%{ldflags} -fprofile-use=$(realpath %{name}-llvm.profdata)" \
%endif
%cmake \
	-DWITH_SANITIZERS=ON \
	-DINSTALL_LIB_DIR=%{_libdir} \
